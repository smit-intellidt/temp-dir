            ...
            for (var i = 0; i < 7; i++) {
                ...
                for (var j = 0; j <= 6; j++) {
                    ...
                    var pos = this.startingDay - this.options.start_day,
                        ...
                        content = '';
                    var calendar_month = this.month + 1;
                    var div_date = (this.year + "-" + (calendar_month < 10 ? ("0" + calendar_month) : calendar_month) + "-" + (day < 10 ? '0' + day : day));
                    if (day <= monthLength && (i > 0 || j >= p)) {

                        html += '<div class="' + cellClasses + (events[div_date] != undefined ? "event-div" : "") + (today ? ' ci-today ' : '') + '" data-month="' + (this.month + 1) + '" data-day="' + day + '" data-events="0" data-date="' + div_date + '">';
...
var calendar = $('#calendar').ciCalendar({
        ...
        dateClick: function ($el, $contentEl, dateProperties) {
            ...
            // console.clear();
            if ($el.hasClass("event-div")) {
                $(".event-row").removeClass("flash");
                var target = "#event_" + $el.data("date");
                if (screen.width >= 768) {
                    $("#upcoming_events").animate({
                        scrollTop: $("#upcoming_events").scrollTop() - $("#upcoming_events").offset().top + $(target).offset().top,
                    }, 1000, function () {
...
function updateMonthYear() {
    ...
    if (calendar.year == today_date.getFullYear() && calendar.month == today_date.getMonth()) {
        date_to_send = (calendar.year + "-" + (month < 10 ? ("0" + month) : month) + "-" + (today_date.getDate() < 10 ? ("0" + today_date.getDate()) : today_date.getDate()))
    } else {
        date_to_send = (calendar.year + "-" + (month < 10 ? ("0" + month) : month) + "-" + "01");
    }
    getEvents(date_to_send);
...
function hideShowButton() {
    var today_date = new Date();
    if (calendar.month == today_date.getMonth() && calendar.year == today_date.getFullYear()) {
...
function getEvents(date) {
    $.ajax({
        url: $("#event_calendar").data("action"),
        type: 'POST',
        data: {
            event_date: date
        },
        success: function (data) {
            data = $.parseJSON(data)
            $("#upcoming_events").html(data.view);
            events = data.data;
            calendar._generate();
        }
    });
