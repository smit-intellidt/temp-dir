...
@php
$dataTypeContent->{$row->field} = $dataTypeContent->{$row->field} != null ? json_decode($dataTypeContent->{$row->field}) : [];
@endphp
...
<select class="form-control select2" name="{{ $row->field }}[]" multiple>
    @if(isset($options->relationship))
        ...
        @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
            <?php $selected_values = isset($dataTypeContent) ? $dataTypeContent->{$row->field} : []; ?>
