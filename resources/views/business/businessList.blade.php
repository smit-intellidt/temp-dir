            ...
            <div class="panel business_container">
                ...
                <div class="panel-body">
                    <div class="table-responsive" id="business_list_table">
                        <table class="table table-bordered table-striped-col data_table_list" data-url="{{ url('business-list') }}" data-page-size="{{ config('constants.pagination_limit') }}" data-ordering="true">
                            <thead>
                            <tr>
...
@section('jquerysection')
    ...
    <script type="text/javascript">
        ...
        function unapproveBusiness(id) {
            if (id == "") {
                alert("Business not found");
                return false;
            } else {
                if (confirm("Are you sure want to unapprove business?")) {
                    $.ajax({
                        type: 'POST',
                        url: '{!! url("unapprove-business") !!}',
                        data: {
                            'business_id': id
                        },
                        success: function (data) {
                            if (data == "success") {
                                $(".business_container").before('<div class="alert alert-success" id="successMessage">Business unapproved successfully</div>');
                                $("#successMessage").delay(5000).slideUp(300);
                                $('.data_table_list').DataTable().draw();
                            }else{
                                $(".business_container").before('<div class="alert alert-danger" id="errorMessage">Something went wrong</div>');
            ...
            if (id == "") {
                ...
                if (confirm("Are you sure want to delete business account?")) {
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == "success") {
                                $(".business_container").before('<div class="alert alert-success" id="successMessage">Business deleted successfully</div>');
                                $("#successMessage").delay(5000).slideUp(300);
                                $('.data_table_list').DataTable().draw();
                            }else{
                                $(".business_container").before('<div class="alert alert-danger" id="errorMessage">Something went wrong</div>');
