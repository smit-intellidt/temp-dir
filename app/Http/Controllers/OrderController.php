...
class OrderController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if($menu_data){
            if($menu_data->date < $last_date)
                $last_date = $menu_data->date;
        }
        
        $menu_details = MenuDetail::where("date",$last_date)->first();
        $cat_id = array(
            1 => 'BA',
            2 => 'LS',
            7 => 'LD',
        ...
        array_push($table_column[0], array("title" => 'Room No', "field" => 'room_id', "rowspan" => 2));
        ...
        if ($menu_details) {
            ...
            foreach (count($all_items) > 0 ? $all_items : array() as $a) {
                ...
                array_push($table_column[1], array("title" => $title, "tooltip" => $a->item_name, "field" => $title));
...
// Next Get or Paginate the actual content from the MODEL that corresponds to the slug DataType
        ...
        foreach ($dataTypeContent as $d) {
            $comment_data = RoomDetail::where("id", $d->room_id)->whereRaw("special_instrucations != \"\" ")->first();
            if ($comment_data) {
                $d->comment = $comment_data->special_instrucations;
            } else {
                $orderwise_comment = SpecialInstructionDetail::where("room_id", $d->room_id)->where("date", $d->date)->first();
                if ($orderwise_comment) {
                    $d->comment = $orderwise_comment->comment;
                }
            }
        }
...
// Define orderColumn
    ...
    public function reportList(Request $request)
    {
        ...
        if ($menu_details) {
            ...
            // array_push($table_column[1],  array("title" => ""));
            foreach (count($total) > 0 ? $total : array() as $k => $v) {
                array_push($table_column[1], array("title" => $v));
            }
        }
        return json_encode(array(
            "result" => array("rows" => $final_array), "columns" => $table_column, "total" => $total
        ));
    }
...
//****************************************
    ...
    public
    function remove_media(Request $request)
    {
        try {
            // GET THE SLUG, ex. 'posts', 'pages', etc.
     ...
     */
    protected
    function cleanup($dataType, $data)
    {
     ...
     */
    public
    function deleteBreadImages($data, $rows, $single_image = null)
    {
        $imagesDeleted = false;
