...
class MenuController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $dataTypeContent = $this->resolveRelations($dataTypeContent, $dataType, true);
         if ($dataTypeContent->items != "") {
            $item_string = "";
            $items = json_decode($dataTypeContent->items, true);
            $category = array();
            foreach (count($items) > 0 ? $items : array() as $key => $i) {
                $items_data = array();
                if (count($i) > 0) {
                    $data = ItemDetail::select("item_name","cat_id")->whereIn("id", $i)->get();
                    ...
                    $item_string .= "<strong>" . ucfirst($key) . " : </strong>";
                    foreach (count($items_data) > 0 ? $items_data : array() as $key1 => $value){
                        $item_string .= "<ul><li>".$category[$key1]."</li><ul>".implode("",$value)."</ul></ul>";
                    }
                }
            }
            $dataTypeContent->items = $item_string;
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $categories = CategoryDetail::selectRaw("id,cat_name,type")->orderBy("id")->get();
        $all_categories = array();
        foreach (count($categories) > 0 ? $categories : array() as $c) {
            $type = intval($c->type);
            if ($type == 1) {
                if (!isset($all_categories['breakfast'])) {
                    $all_categories['breakfast'] = array();
                }
                array_push($all_categories['breakfast'], $c);
            } else if ($type == 2) {
                if (!isset($all_categories['lunch'])) {
                    $all_categories['lunch'] = array();
                }
                array_push($all_categories['lunch'], $c);
            } else if ($type == 3) {
                if (!isset($all_categories['dinner'])) {
                    $all_categories['dinner'] = array();
                }
                array_push($all_categories['dinner'], $c);
            }
        }
        $items = ItemDetail::selectRaw("id,item_name,cat_id,item_image")->orderBy("item_name","asc")->get();
        $all_items = array();
        foreach (count($items) > 0 ? $items : array() as $i) {
            if (!isset($all_items[$i->cat_id])) {
                $all_items[$i->cat_id] = array();
            }
             array_push($all_items[$i->cat_id], array("item_id" => $i->id, "item_name" => $i->item_name, "truncated_name" => substr($i->item_name, 0, 50) . "...", "image" => $i->item_image != "" ? Voyager::image($i->item_image) : "http://hamiltondinnerapp.intellidt.com/uploads/settings/March2023/37s3zyTHNasT40MHKuiN.png"));
        }

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','all_categories','all_items'));
    }
